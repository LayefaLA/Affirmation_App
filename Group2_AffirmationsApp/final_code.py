...
# Creates individual home page
    def home_page(self):
        ...
        lb = tk.Label(home_frame, bg=self.bk_ground)
        lb.pack()
        home_frame.pack(pady=20)

        home_header = Image.open("Images/home_page.jpg")
        resized_about = home_header.resize((400, 350))
        about_image = ImageTk.PhotoImage(resized_about)

        lb = tk.Label(home_frame, image=about_image, bg=self.bk_ground)
    ...
    def about_page(self):
        ...
        """, font=('Arial', 12), fg="#158aff", bg="#F0D5E7")
        lb.pack()
